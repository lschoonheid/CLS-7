...
def plot_phase_diagram_robustness():
    ...
    plt.text(epsilons.min() + 0.01, alphas.max() - 0.05, "THE HOARDER\n(High Cost)", 
             color='#8B0000', ha='left', va='top', bbox=box_style, **text_args)
    plt.text(epsilons.max() - 0.01, alphas.min() + 0.05, "THE MINIMALIST\n(High Risk)", 
             color='#006400', ha='right', va='bottom', bbox=box_style, **text_args)
    plt.text(np.mean(epsilons), np.mean(alphas), "GOLDILOCKS ZONE\n(Robust Stability)", 
